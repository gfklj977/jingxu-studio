...
export async function preparePublish(projectId: number, platform: PublishPlatform): Promise<void> { const response = await fetch(`/api/projects/${projectId}/publish-drafts/${platform}/prepare`, { method: 'POST' }); if (!response.ok) throw new Error(response.status === 409 ? '请先生成成片、封面和投放草稿' : '发布准备失败') }
