...
export function ProductionWorkspace({ projectId }: { projectId: number }) {
  ...
  return <section className="content-section production-workspace">
    ...
    {job && <div className="job-status"><div><strong>任务 #{job.id}</strong><Tag color={job.status === 'CANCELLED' ? 'default' : 'processing'}>{job.status === 'QUEUED' ? '排队中' : job.status === 'CANCELLED' ? '已取消' : job.status}</Tag></div>{job.stages.map((item) => <span key={item.name}>{stageLabels[item.name]} <small>{item.status}</small></span>)}</div>}
